...
class StateManager:
    ...
    def render_scenes_switch( self, current_scene ):
        ...
        for i in range( len( self.scenes) ):
            ...
            t_color = '#3b3b3b' if i == current_scene else '#8b8b8b'
            t_font  = tkFont.Font( family='Helvetica', size=20 )
            self.switch_cv.create_rectangle( offset, 0,
                                             offset + 29, 29,
                                             fill=f_color, outline='' )
            self.switch_cv.create_text( offset+15, 15, 
					text=str(i+1), 
					fill=t_color,
					font=t_font )

    def click( self, event ):
        self.current_scene = event.x / 30
...
state = StateManager( state_frame, tracks )
root.bind( '<Key>', state.key )
