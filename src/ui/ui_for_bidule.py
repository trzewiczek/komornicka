...
class Track:
    def __init__( self, master, inx ):
        ...
        self.speakers = [
            {
                'x':   0, 'y':   0, 'a': 'nw',
                't': Toggle( frame, on=True, url=self.base_url + '/Gate_1/Gate' )
            },
            {
                'x': 100, 'y':   0, 'a': 'ne',
                't': Toggle( frame, on=True, url=self.base_url + '/Gate_2/Gate' )
            },
            {
                'x':   0, 'y':  50, 'a':  'w',
                't': Toggle( frame, on=True, url=self.base_url + '/Gate_3/Gate' )
            },
            {
                'x': 100, 'y':  50, 'a':  'e',
                't': Toggle( frame, on=True, url=self.base_url + '/Gate_4/Gate' )
            },
            {
                'x':   0, 'y': 100, 'a': 'sw',
                't': Toggle( frame, on=True, url=self.base_url + '/Gate_5/Gate' )
            },
            {
                'x': 100, 'y': 100, 'a': 'se',
                't': Toggle( frame, on=True, url=self.base_url + '/Gate_6/Gate' )
            }
        ]
    ...
    def send_state( self ):
        ...
        for i, s in enumerate( self.speakers ):
            msg.append( 0.97 - self.get_distance( s['x'], s['y'] ) )
    ...
    def move_master( self, event=None ):
        ...
        # prepare message
        msg = OSC.OSCMessage()
        msg.setAddress( self.base_url + '/MASTER/Value' )
